...
* [**Creating a smart account**](getting-started/essentials/creating-a-smart-account.mdx)
* [**Handling async execution**](getting-started/essentials/handling-async-execution.mdx)
* [**Receiving external transactions**](getting-started/essentials/receiving-ext-transactions.mdx)
...
* It can [**accept external transactions and verify them**](getting-started/essentials/receiving-ext-transactions.mdx)
* It will [**limit what transactions can call**](tools/sol-library/func-modifiers.mdx) some of its functions
...
Contract 1 stores its owner's `pubkey`, which is useful for verifying external transactions. The contract also contains the address of the retailer contract. This is done so that only the retailer contract can order new products.
...
The contract also conntains a simple mechanism for verifying external transactions:
...
Last but not least, Contract 2 can produce new products (but only via an internal transaction sent from the retailer):

...
The `Nil.sol` contract also exposes the `awaitCall()` function.
...
The contract is designed to call [**the`Counter` contract**](../nil-101.mdx#flow-1) asynchronously and store the returned value inside the `result` variable.

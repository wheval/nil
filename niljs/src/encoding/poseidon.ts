...
const hashBytes = (txn: Uint8Array): bigint => {
  const frameSize = 16;
  const inputs = new Array(frameSize).fill(BigInt(0));
  let dirty = false;
  let hash = 0n;
  let k = 0;
  for (let i = 0; i < Number.parseInt(`${txn.length / SPONGE_CHUNK_SIZE}`); i += 1) {
    dirty = true;

    inputs[k] = beBuff2int(txn.slice(SPONGE_CHUNK_SIZE * i, SPONGE_CHUNK_SIZE * (i + 1)));
    if (k === frameSize - 1) {
  ...
  if (txn.length % SPONGE_CHUNK_SIZE !== 0) {
    const buff = new Uint8Array(SPONGE_CHUNK_SIZE);
    const slice = txn.slice(
      Number.parseInt(`${txn.length / SPONGE_CHUNK_SIZE}`) * SPONGE_CHUNK_SIZE,
    );
    slice.forEach((v, idx) => {
      buff[idx] = v;
    });

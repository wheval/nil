...
func TestByzantineBehaviour(t *testing.T) { //nolint:maintidx
	t.Parallel()
	...
	t.Run("malicious hash in proposal", func(t *testing.T) {
		...
		require.NoError(t, cluster.progressToHeight(20*time.Second, 2))
	...
	t.Run("malicious hash in prepare", func(t *testing.T) {
		...
		cluster.makeNByzantine(int(cluster.maxFaulty()))
		require.NoError(t, cluster.progressToHeight(10*time.Second, 2))
		assert.Equal(t, uint64(2), cluster.latestHeight)
	})
	...
	t.Run("malicious +1 round in proposal", func(t *testing.T) {
		...
		cluster.makeNByzantine(int(cluster.maxFaulty()))
		require.NoError(t, cluster.progressToHeight(40*time.Second, 2))
		assert.Equal(t, uint64(2), cluster.latestHeight)
	})
	...
	t.Run("malicious +1 round in rcc", func(t *testing.T) {
		...
		cluster.makeNByzantine(int(cluster.maxFaulty()))
		require.NoError(t, cluster.progressToHeight(30*time.Second, 2))
		assert.Equal(t, uint64(2), cluster.latestHeight)
	})

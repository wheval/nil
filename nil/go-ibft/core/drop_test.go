...
func TestDropAllAndRecover(t *testing.T) {
	...
	require.NoError(t, err, "unable to reach height: %w", err)
	...
	require.NoError(t, cluster.progressToHeight(5*time.Second, 2))
	...
	require.NoError(t, cluster.progressToHeight(5*time.Second, 10))
	assertValidInsertedBlocks(t, insertedBlocks) // Make sure the inserted blocks are valid
}
...
func assertNInsertedBlocks(t *testing.T, n int, blocks [][]byte) {
	...
	assert.Equal(t, n, writtenBlocks)
}
...
func TestMaxFaultyDroppingMessages(t *testing.T) {
		...
		func(c *cluster) {
			for _, node := range c.nodes {
				...
				node.core = NewIBFT(
					...
					&mockTransport{multicastFn: func(message *proto.IbftMessage) {
						if currentNode.faulty && rand.Intn(100) < 50 { //nolint:gosec
							return
						}
	...
	require.NoError(t, cluster.progressToHeight(40*time.Second, 5))
...
func TestDropMaxFaultyPlusOne(t *testing.T) {
	...
	require.Error(t, cluster.progressToHeight(2*time.Second, 10))
	assert.Equal(t, uint64(5), cluster.latestHeight)
...
func TestDropMaxFaulty(t *testing.T) {
	...
	require.NoError(t, cluster.progressToHeight(20*time.Second, 10))
	assert.Equal(t, uint64(10), cluster.latestHeight)
}

...
func ListPeers(networkManager *network.BasicManager, shardId types.ShardId) []network.PeerID {
...
func PublishBlock(
	ctx context.Context, networkManager *network.BasicManager, shardId types.ShardId, block *types.BlockWithExtractedData,
) error {
...
func RequestBlocks(ctx context.Context, networkManager *network.BasicManager, peerID network.PeerID,
...
func SetRequestHandler(
	ctx context.Context, networkManager *network.BasicManager, shardId types.ShardId, database db.DB, logger logging.Logger,
) {
	if networkManager == nil {
		// we don't always want to run the network
		return

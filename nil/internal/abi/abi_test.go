...
func ExampleJSON() {
	...
	out, err := abi.Pack("isBar", types.HexToAddress("01"))
	if err != nil {
		panic(err)
	}
...
func TestInputVariableInputLength(t *testing.T) {
	...
	exp := append(offset, append(length, value...)...) //nolint:gocritic
...
func TestInputFixedArrayAndVariableInputLength(t *testing.T) {
	...
	exp = append(stroffset, fixedarrinvalue1...) //nolint:gocritic
	exp = append(exp, fixedarrinvalue2...)
	exp = append(exp, dynarroffset...)
	exp = append(exp, append(strlength, strvalue...)...)
	dynarrarg := append(dynarrlength, dynarrinvalue1...) //nolint:gocritic
	...
	exp = append(stroffset, fixedarrin1value1...) //nolint:gocritic
...
func TestUnpackEventIntoMap(t *testing.T) {
	...
	if receivedMap["amount"].(*big.Int).Cmp(expectedReceivedMap["amount"].(*big.Int)) != 0 { //nolint:forcetypeassert
		t.Error("unpacked `received` map does not match expected map")
	}
	if !bytes.Equal(receivedMap["memo"].([]byte), expectedReceivedMap["memo"].([]byte)) { //nolint:forcetypeassert
		t.Error("unpacked `received` map does not match expected map")
	}
...
func TestUnpackMethodIntoMap(t *testing.T) {
	...
	if sendMap["amount"].(*big.Int).Cmp(big.NewInt(1)) != 0 { //nolint:forcetypeassert
...
func TestUnpackRevert(t *testing.T) { //nolint:tparallel
	t.Parallel()
	...
	for index, c := range cases { //nolint:paralleltest
		t.Run(fmt.Sprintf("case %d", index), func(t *testing.T) {
			input, _ := hex.DecodeString(c.input)
			got, err := UnpackRevert(input)
			if c.expectErr != nil {
				if err == nil {

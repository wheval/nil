...
task(
  ...
  const initTx = await pair.write.initialize([token0.address, token1.address]);
  await waitTillCompleted(client, initTx, { waitTillMainShard: true });
  ...
  const hash = await router.write.addLiquiditySync(
    [
      pairAddress,
      smartAccount.address,
      mintToken0Amount,
  ...
  await waitTillCompleted(client, hash, { waitTillMainShard: true });
  console.log("Liquidity added to the pair contract. Tx - ", hash);
  ...
  console.log("Balance of token0 before swap:", balanceToken0Before.toString());
  console.log("Balance of token1 before swap:", balanceToken1Before.toString());

  // Execute the swap
  console.log("Executing swap...");

  // Send token0 to the pair contract
  const hash2 = await router.write.swapExactTokenForTokenSync(
    [pairAddress, 1, smartAccount.address],
    {
      tokens: [
        {
          id: token0.address,
  ...
  const hash3 = await router.write.removeLiquiditySync(
    [pairAddress, smartAccount.address, 100, 100],
    {
      tokens: [
        {
          id: pairAddress,

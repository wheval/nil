...
import { archiveConfig, isValidAddress, isValidBytes32, loadConfig, NetworkConfig, saveConfig } from "./config/config-helper";
...
const deployNilVerifier: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  const { deployments, getNamedAccounts, ethers, network } = hre;
  const { deploy } = deployments;
  const networkName = network.name;

  console.log(`Deploying NilVerifier to ${network.name}...`);

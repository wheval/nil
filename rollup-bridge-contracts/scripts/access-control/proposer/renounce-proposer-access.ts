...
export async function renounceProposerAccess(proposerAddress: string) {
    ...
    if (!isAProposerResponse) {
        throw new Error(`account: ${proposerAddress} is not a proposer. so renounceProposerAccess cannot be initiated`);
    }
    ...
    console.log(`Proposer access renounced by ${proposerAddress}`);
    ...
    if (isAProposerResponse) {
        throw new Error(`renounceProposerAccess failed. account: ${proposerAddress} is still a proposer.`);
    }
}

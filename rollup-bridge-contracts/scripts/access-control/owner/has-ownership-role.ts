...
export async function hasOwnershipRole(account: string) {
  ...
  if (hasOwnershipRoleIndicator) {
    console.log(`account: ${account} is an owner for rollupContract: ${config.nilRollupProxy} on network: ${networkName}`);
  } else {
    console.log(`account: ${account} doesnot have owner-role for rollupContract: ${config.nilRollupProxy} on network: ${networkName}`);
  }
}
